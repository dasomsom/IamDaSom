...
<head>
	...
	<? include_once('../_inc/datatable.php'); ?>
</head>
<body>
	<!-- menu bar-->
				...
				<div class="sub-bar clearfix">
					<? for( $i=0; $i<count($prodtitle)-1; $i++) {?>
						<ul class="pull-left">
							...
							<? for($j=0; $j<count($prodtitle['table']); $j++) {?>
								<li><a href="tableshop.php?tabletab=<?=$j+1?>"><?=$prodtitle['table'][$j]?></a></li>
							<?}?>
						</ul>
					<?}?>
					<ul class="pull-right mr15" id="sub_bt">
						<li><a href="<?=$gnb['shop']['title1url']?>" class="fl16 greenb16"><?=$gnb['shop']['title1']?></a></li>
